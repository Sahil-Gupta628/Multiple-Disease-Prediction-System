...
def predict_diabetes(diabetes_model):
    ...
    st.image("images/DiabetesImg.jpg", width=600)

    st.subheader('General Patient Information')
    pregnancies = st.number_input(
        'Number of Pregnancies', min_value=0, max_value=17, value=3, step=1)
    glucose = st.number_input('Glucose Level (mg/dl)',
                              min_value=0, max_value=200, value=120, step=1)
    blood_pressure = st.number_input(
        'Blood Pressure (mmHg)', min_value=0, max_value=122, value=70, step=1)
    skin_thickness = st.number_input(
        'Skin Thickness (mm)', min_value=0, max_value=110, value=20, step=1)
    insulin = st.number_input(
        'Insulin Level (mu U/ml)', min_value=0, max_value=846, value=79, step=1)
    bmi = st.number_input('BMI', min_value=0.0,
                          max_value=67.1, value=32.0, step=0.1)
